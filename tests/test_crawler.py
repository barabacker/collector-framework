"""Crawler is declarative: it builds requests and describes items, nothing more."""
...
from collector import Crawler, CrawlerContext, Settings
...
def _crawler() -> _TwoPages:
    return _TwoPages(CrawlerContext(http=FakeHttp()))


def test_a_crawler_keeps_no_run_state():
    """Counters and failures belong to the Crawl; a crawler is reusable and inert."""
    crawler = _crawler()
    assert not hasattr(crawler, 'item_count')
    assert not hasattr(crawler, 'errors')
    assert not hasattr(crawler, 'crawl')
...
def test_the_context_carries_only_fields_that_have_a_reader():
    ...
    assert [f.name for f in fields(CrawlerContext)] == ['http', 'params', 'sink', 'log']


def test_request_defaults_its_callback_to_parse():
    crawler = _crawler()
    req = crawler.request(PAGE_2)
...
async def test_start_requests_can_be_overridden():
    ...
    crawler = _PostStart(CrawlerContext(http=FakeHttp()))
    reqs = [req async for req in crawler.start_requests()]
...
async def test_log_is_a_noop_without_a_log_callable():
    await _crawler().log('nothing blows up')
...
async def test_process_item_is_a_noop_by_default():
    await _crawler().process_item({'anything': 1})
